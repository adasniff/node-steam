...
Steam._processProto = function (proto) {
  ...
  (function deleteNulls(proto) {
    for (var field in proto)
      if (proto[field] == null) delete proto[field];
      else if (typeof proto[field] == "object") deleteNulls(proto[field]);
  })(proto);
  return proto;
};
...
Steam.servers = require("../servers").response.serverlist.map(function (
  server
) {
...
SteamClient.prototype._send = function (header, body, callback) {
  ...
  if (this._connection) {
    ...
    this.emit("error", new Error("client connection lost"));
  }
};

SteamClient.prototype.send = function (header, body, callback) {
  // ignore any target job ID
  if (header.proto) delete header.proto.jobid_target;
  else delete header.targetJobID;
...
SteamClient.prototype._netMsgReceived = function (data) {
  ...
  if (eMsg in handlers) handlers[header.msg].call(this, body);

  if (sourceJobID != "18446744073709551615") {
    var callback = function (header, body, callback) {
      if (header.proto) header.proto.jobid_target = sourceJobID;
      else header.targetJobID = sourceJobID;
      this._send(header, body, callback);
    }.bind(this);
  }
  ...
  else this.emit("message", header, body, callback);
};
